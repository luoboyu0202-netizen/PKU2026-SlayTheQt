...
│   ├── m_handCursor (QGraphicsPixmapItem)     — 手指，悬停售卖品时移入，离开时移出屏幕
│   │   ├── 悬停售卖品 → 手指移动到该物品上方边缘（连续动画）
│   │   └── 鼠标离开所有售卖品 → 手指移回屏幕上方消失（连续动画）
│   ├── m_cardSlots[7]                          — 7个卡牌槽位
│   │   ├── 第一排5张，第二排2张
│   │   ├── 每张显示：费用/插画/名称/效果/价格
│   │   ├── 随机1张附打折标签 (label.jpg)
│   │   ├── 悬停 → 放大自身（连续缩放动画，非高亮变色）
│   │   ├── 点击 → 金币足够则购买，购买后变为光点飞向右上角（带拖尾）
│   │   └── 购买后原位置清空，不保留任何残留
│   ├── m_relicSlots[3]                         — 3个遗物槽位
│   │   ├── 位于第二排卡牌右侧
│   │   ├── 最右(m_relicSlots[2])必为商店专属遗物
│   │   ├── 悬停 → 放大自身（连续缩放动画）
│   │   └── 点击购买 → 购买后变为光点飞向右上角（带拖尾）
│   ├── m_removeButton (QGraphicsPixmapItem)    — 删牌入口(remove.jpg)
│   │   ├── 位于遗物右侧
│   │   ├── 点击 → 弹牌组选择 → 确认删除 → 扣费
│   │   └── 使用后替换为 m_soldoutPlaceholder (soldout.jpg)（唯一保留占位的项目）
...
阶段二：地毯展开，卡牌/遗物/删牌/离开飘带依次出现
    ↓
┌────────────────── 购物循环 ──────────────────┐
│ 鼠标悬停售卖品 → 手臂从屏幕外移入该物品边缘      │
│                  (连续动画，移动到悬停物品上方)  │
│ 鼠标离开售卖品 → 手臂移回屏幕上方消失（连续动画）│
│ 悬停卡牌/遗物 → 物品放大（连续缩放，非高亮变色） │
│ 点击卡牌/遗物 → 金币够 → 物品化为光点            │
│               → 光点带拖尾飞向右上角离开屏幕      │
│               → 购买后原位置完全清空              │
│ 金币不足 → 物品保持原样，无响应                  │
│ 点击删牌 → 弹牌组选择 → 确认删除 → 扣费          │
│          → remove.jpg 变为 soldout.jpg(唯一占位) │
│ 悬停离开飘带 → 手臂移入飘带上方                  │
