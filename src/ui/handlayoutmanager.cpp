...
void HandLayoutManager::recalculateLayout() {
    ...
    // 优雅的完美扇形算法喵！
    qreal spacing = 150.0 - (count * 5.0); // 牌越多，挤得越紧
    qreal totalWidth = (count - 1) * spacing;
    qreal startX = 960.0 - totalWidth / 2.0; // 以屏幕中心 960 为对称轴

    for (int i = 0; i < count; ++i) {
        CardItem* item = m_handItems[i];

        // 🔴【护体神功】：如果它在天上悬浮施法，排版管家不许动它！！
        if (item->isSuspended()) continue;

        qreal targetX = startX + i * spacing;
        // 越靠边，角度越大；中心的牌角度为 0
        qreal angle = (targetX - 960.0) / 12.0;
        // 越靠边，位置越往下沉，形成优美的弧线
        qreal targetY = 920.0 + qAbs(angle) * 3.0;
